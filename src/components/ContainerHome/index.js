...
import CardSemBotao from "../CardSemBotao"
import "./style.css"

const ContainerHome = ()=>{

    const [totalEntrada,setTodalEntrada] = useState()
    const [totalSaida,setTodalSaida] = useState()
    const [totalSaldo,setTodalSaldo] = useState()
    ...
    const lista = [
        ...
        useEffect(()=>{
            ...
            lista.forEach((transacoes) => {
                totalEntrada += transacoes.entrada
                totalSaida += transacoes.saida
                totalSaldo += transacoes.total
            });

            setTodalEntrada(totalEntrada)
            setTodalSaida(totalSaida)
            setTodalSaldo(totalSaldo)
    ...
    return(
        <div className="containerHome">
             <Card
          titulo="Entradas"
          saldo={`R$: ${totalEntrada}`}
          textoBotao={"Adicionar Entrada"}       
          />
          <Card
          titulo="Saídas"
          saldo={`R$: ${totalSaida}`}
          textoBotao={"Adicionar Saídas"}       
          />
          <CardSemBotao
          titulo="Saldo"
          saldo={`R$: ${totalSaldo}`}
